...
import { useEditarInvitado } from '../hooks/useInvitadoMutations';
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <KeyboardAvoidingView
        ...
        {/* El `key` remonta el formulario al abrirlo, así arranca con los datos
            del invitado sin un efecto que sincronice estado. */}
        {visible && invitado && (
          <Formulario
            key={`${invitado.id}-${invitado.nombre}-${invitado.correo}`}
            reservaId={reservaId}
            invitado={invitado}
            onClose={onClose}
            onSaved={onSaved}
...
function Formulario({ reservaId, invitado, onClose, onSaved }: FormularioProps) {
  const styles = useStyles();
  const { colors } = useTheme();
  const [nombre, setNombre] = useState(invitado.nombre);
  const [correo, setCorreo] = useState(invitado.correo);
  const [error, setError] = useState('');

  const editar = useEditarInvitado(reservaId);
  ...
  const handleGuardar = () => {
    ...
    setError('');
    editar.mutate(
      { invitadoId: invitado.id, input: { nombre: nombreTrim, correo: correoTrim } },
      {
        onSuccess: () => {
          onSaved();
          onClose();
        },
        onError: err => setError(err.message || 'No se pudo editar el invitado.'),
      },
    );
  };
